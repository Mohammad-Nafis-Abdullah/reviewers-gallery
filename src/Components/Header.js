...
const Header = () => {
    ...
    return (
        <div className={`${menu[1]} shadow-md fixed top-0 left-0 right-0 z-50 bg-black/10 sm:bg-white`}>
            <section className={`duration-500 container mx-auto flex justify-between p-5 sm:justify-center bg-white`}>
                <img className='sm:hidden w-10 h-12 active:scale-90 transition cursor-pointer' src={menu[2]} alt="" onClick={closeMenu} />
                <nav className={`${menu[0]} transition-[height] overflow-hidden duration-300 grow px-2 flex flex-col gap-5 justify-start items-end sm:flex-row sm:justify-center sm:items-center sm:px-0 sm:grow-0 sm:h-10 sm:overflow-visible sm:transition-none`}>
